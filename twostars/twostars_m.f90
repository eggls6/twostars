...
public::insolation2  !Same as insolation1 but with direct input of stellar effective temperatures and radii

public::insolation3  !Same as insolation2 but takes input stellar spectra instead of calculating blackbody spectra, and outputs time dependent weighed spectra,
                     !    net weighted spectrum reaching planet, and set of time dependent weight factors
public:: insolation4 !Same as insolation3 with added support for non-zero initial orbit eccentricity for planets
...
!*****************************************************************************************************************************************
subroutine insolation4(spectra,mass,rs,semia,e1,e2,man,spin,t,nbands,bands,bndflux,decl,phi,d2,dr1,dr2,weights,e2t,w1,w2)
!-----------------------------------------------------------------------------------------------------------------------------------------
! Determine the top-of the atmosphere insolation a circumbinary (P-type) planet receives in
! specified wave-length bands
!
...
!*)planetary and stellar dynamics are solved analytically using relations from Georgakarakos & Eggl 2014
!
! written by Siegfried Eggl 20200628
!
! dependencies: pconst, pdist, getspin, planetorient, intplanckwn, transitfactor
!-----------------------------------------------------------------------------------------------------------------------------------------
! Input
...
! rs(1:2)        		... [m] 			stellar (mean) radii                                         !ADZ 20190815 REMOVE teff
! semia(1:2)     		... [au]   	a1, a2: 	semimajor axes of the binary (inner) and planetary (outer) orbit. 0<a1<a2
! e1             		... [] 				eccentricity of binary star's orbit  0 <= e1 < 1
! e2                ... []      initial eccentricity of the planetary orbit 0 <= e2 < 1 / short period eccentricity evolution only available for e2=0
! man(1:2)       		... [rad]	ma10, ma20:	inital positions of the binary/planet on its orbit (initial mean longitudes), range [0,360[ deg
! spin(1:4)      		... [mixed] 	(1) obl [deg]	obliquity (ref: z-axis = normal to the orbital plane), range [0,180[ deg
! 						(2) pre [deg]	angle of precession (ref: x-axis = along initial pericenter of binary), range [0,360[ deg
! 						(3) pp  [D]     rotation period of the planet
...
! dr2(1:2)                      [au]
! weights(1:2)                  []                      ... scaling factor by which to weight each stars spectrum at each time step (due to distance to star and area blocked during eclipses)  !ADZ CHANGE 8/8/19
! e2t                           []          ... eccentricity of outer orbit at time t
! w1                            [rad]       ... argument of pericenter of the inner orbit
! w2                            [rad]       ... argument of pericenter of the outer orbit
!-----------------------------------------------------------------------------------------------------------------------------------------------------------
 implicit none
 !input
 integer,intent(in)::nbands
 real(kind=dp),intent(in)::e1,e2,t
 real(kind=dp),dimension(1:2),intent(in)::rs,semia,man        !ADZ 20190815 REMOVE teff
 real(kind=dp),dimension(1:3),intent(in)::mass
 real(kind=dp),dimension(1:4),intent(in)::spin
 real(kind=dp),dimension(1:2,1:nbands,1:2),intent(in)::bands
 ...
 real(kind=dp),dimension(1:2),intent(out)::d2,dr1,dr2         !ADZ CHANGE 20190717 (move here from local variables)
 real(kind=dp),dimension(1:2), intent(out)::weights           !ADZ CHANGE 20190801
 real(kind=dp),intent(out)::e2t,w1,w2                         !eccentricity of the outer orbit at time t, argument of pericenter of the inner and outer orbit [rad]
 ...
 !dynamical evolution of the planet and the binary
 !---------------------------------------------------------------------
 call pconst2(semia(1),semia(2),e1,e2,man(1)*deg2rad,man(2)*deg2rad,mass(1),mass(2),mass(3),c,cs)
 call pdist(semia(1),semia(2),e1,mass(1),mass(2),mass(3),c,cs,t,.true.,e2,r0,r1,r2,e2t,w1,w2)
...
real(kind=dp)::c(1:11),cs(1:6)
 ...
 cs(3)=3._dp/4._dp*c(1)*m2*sqrt(a1**3*(1._dp-e12))/(sqrt(m0+m1)*a2**3) &
       +3._dp*c(1)**3*(m0+m1)**1.5_dp/(c(3)*a1**2.5_dp*(1._dp-e12))
...
real(kind=dp)::c(1:11),cs(1:6)
 ...
 !calculate secular amplitudes
 cs(1)=3._dp/8._dp*c(1)*sqrt(c(5))*m0*m1*a1**2*(2._dp+3._dp*e12)/((m0+m1)**2*a2**3.5_dp)
 cs(2)=15._dp/64._dp*c(1)*sqrt(c(5))*m0*m1*(m0-m1)*a1**3*e1* &
       (4._dp+3._dp*e12)/((m0+m1)**3*a2**4.5_dp)
 cs(3)=3._dp/4._dp*c(1)*m2*sqrt(a1**3*(1._dp-e12))/(sqrt(m0+m1)*a2**3)+ &
       3._dp*c(1)**3*(m0+m1)**1.5_dp/(c(3)*a1**2.5_dp*(1._dp-e12))


 !short period contribution to emax
 emaxsp= c(6)*c(11)**(-4._dp/3._dp)*(1.5_dp+17._dp/2._dp*e12+ &
         1._dp/c(11)*(3._dp+19._dp*e1+ &
...
!**********************************************************************************************************
subroutine pconst2(a1,a2,e1,e2,ma10,ma20,m0,m1,m2,c,cs)
!--------------------------------------------------------------------------------------------------------------------------------
!generates constants necessary for describing the outer (planetary) eccentricity evolution
!
! written by Siegfried Eggl  20140304
...
!real::
!a1,a2               [au]   semimajor axes of the inner and outer orbit (for P-type motion a1:binary, a2:circumbinary planet)
!e1                  []     eccentricity of the inner orbit (P-type motion: eccentricity of the orbit of the binary)
!e2                  []     initial eccentricity of the outer orbit (P-type motion: eccentricity of the orbit of the planet)
!ma10,ma20           [rad]  initial phases (mean longitude of the inner and mean longitude of the outer orbit)
!m0,m1,m2            [Msun] masses of the primary, secondary, third body (P-type motion: binary star 1, binary star2, planet.)
!--------------------------------------------------------------------------------------------------------------------------------
!Output
...
!---------------------------------------------------------------------------------------------
implicit none
real(kind=dp),intent(in)::a1,a2,e1,e2,m0,m1,m2,ma10,ma20
real(kind=dp),intent(out)::c(1:14),cs(1:6)
!local
real(kind=dp)::e12,e2spx,e2spy,ea0
real(kind=dp)::emaxsp,emaxsec
 ...
 ! Match negative short period ecc amplitudes to zero out with secular terms
 ! if (e2==0.d0) then
 !   !call short period terms at t=0 to get constants
 !   call pesp(e1,0._dp,c,e2spx,e2spy)
 !
 ...
 if (e2<0.5d0) then
   ...
   ! C1
   cs(4)=e2-e2spx-cs(6)
   !cs(4)=e2-(sqrt(e2spx**2+e2spy**2))-cs(6)
   !cs(4)=e2-(emaxsp-sqrt(e2spx**2+e2spy**2))-cs(6)
   ! C2
   !cs(5)=e2spy
   cs(5)=e2spy
 ! Determine constants for secular evolution from e2
 else
   ! C3 = K2/(K1-K3)
   cs(6) = cs(2)/(cs(1)-cs(3))
   ! C1
...
!**************************************************************************************************
subroutine peouter2(cs,t,e2x,e2y)
!-------------------------------------------------------------------------------------------------------
!calculates outer eccentricity vector through secular terms only
!
!written by Siegfried Eggl  20200628
!
!dependencies: pesp, pesec
